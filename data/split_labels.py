...
def split_data_labels(options: ExperimentOptions):
    labels = np.array([f.replace('.png', '') for f in os.listdir(options.dataset_location) if f.endswith('.png')])
    np.random.seed(42)
    ...
    if options.n_unseen_shapes == 0:
        ...
        if options.n_unseen_shapes == 1:
            ...
            valid_labels = set([l for l in other_labels if any([all([s in l for s in c]) for c in combinations(separated, 2)])])
    ...
    return train_labels, list(valid_labels)
